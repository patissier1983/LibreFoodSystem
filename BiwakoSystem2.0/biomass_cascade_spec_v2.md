...
| P3（温水ループ・糖化） | P2凝縮水の顕熱 | 50〜55℃ | ジャケット付き攪拌槽（SSF糖化フェーズ） | （P2より融通） |
| P4（温水ループ・発酵） | P2凝縮水＋冷却水・チラー | 30〜40℃ | ジャケット付き攪拌槽（SSF発酵フェーズ・冷却主体） | （冷却側） |
...
| 固液分離 | フィルタープレス等（独立設備） | P2補助（中圧・110〜120℃） |
...
| CO₂炭酸化・CaCO₃固定 | 炭酸化タンク（スラリー式） | 常温・常圧（追加熱源不要） |

### 3-3. 工程フロー

```
原料投入（木質系バイオマス：パレット廃材・間伐材等）
　　↓ 異物除去・粉砕
【水蒸気爆砕装置】P1高圧蒸気 160〜180℃（0.62〜1.00 MPa）
　高温高圧加熱 → 急速圧力開放 → セルロース解繊・リグニン軟化
　　↓
【フィルタープレス等】P2中圧蒸気補助 110〜120℃
　固液分離
　　　├─ リグニン → ロータリードライヤー（ボイラー排ガス）→ 含水率15%以下 → ボイラー燃料
　　　└─ セルロース液 → SSFへ
　　↓
【ジャケット付き攪拌槽】SSF（同時糖化発酵）
　温水ループ 50〜55℃（P2凝縮水を熱源）で糖化スタート
　　↓ 冷却水で段階的に降温
　温水ループ／冷却水 30〜40℃＋冷却水・チラー制御で発酵（発熱反応のため冷却主体）
　処理時間：72〜96時間（参照値）
...
　├─ CaCO₃ → 農地還元（土壌pH改良材・採掘石灰石の代替＝製品）
　│　　　生成量：約613 t-CaCO₃/年（草本系30%・約270 t-CO₂を鉱物化／正味除去はライムLCA次第）
　└─ 余剰CO₂（約1,110 t-CO₂/年）→ ドライアイス製造・CO₂施用（施設園芸）
　　　　　　　　　　　　　　　　　 ※炭素固定効果なし（農業生産性向上・収益化目的）
　 RT-CaCCO消費Ca(OH)₂を発酵CO₂で完全炭酸化（自己完結ループ）
```
...
4. 草本系サブライン（RT-CaCCO）で消費したCa(OH)₂を、SSF発酵排ガス（ほぼ純CO₂ 95〜99%）で常温炭酸化し、CaCO₃（土壌改良材・採掘石灰石の代替）として回収する自己完結ループ（追加熱源不要。生物起源CO₂の鉱物化分は、低炭素ライム使用時に除去クレジット化が可能）
...
| 対象原料 | 草本系（稲わら・麦わら・葦・エリアンサス・バガス等） | 確定 |
...
固液分離は機械的操作が主体。蒸気は粘度低減・温度維持のための補助熱源として利用する。
...
| CO₂無機固定量（粗） | 約270 t-CO₂/年（鉱物化） | 参照値。**正味除去はライムのLCA次第**（下記注意） |

SSF発酵工程の排ガスはほぼ純CO₂（95〜99%）であり、DACのように大気中420 ppmから濃縮する必要がない。RT-CaCCO前処理で消費したCa(OH)₂を発酵CO₂で炭酸化することで、バイオ由来炭素を鉱物として永続固定する自己完結ループが成立する。これはBECCS（Bioenergy with Carbon Capture and Storage）の農村実装版であり、地中貯留インフラを必要としない。
...
生成CaCO₃：約613 t/年 → 農地還元（土壌pH改良材・採掘石灰石の代替）
固定量を増やす場合：前処理必要分を超えて低炭素ライムを追加投入し余剰発酵CO₂を炭酸化すれば、
　　　　　　　　　　 固定量は発酵CO₂上限（約1,380 t）まで拡大可能（ライムLCAの条件は同じ）
地中貯留：理論上可能だが482t規模では圧縮・注入設備の固定費が不釣り合い
　　　　　規模拡大（フェーズ3以降）で改めて検討
```
...
| CaCO₃（炭酸カルシウム） | 農地還元（土壌pH改良材・採掘石灰石の代替）＝**確定的な製品アウトプット** | 約613 t/年 |
| 余剰CO₂ | ドライアイス製造・CO₂施用（施設園芸・収量向上）。炭素固定効果なし | 約1,110 t-CO₂/年 |
| CO₂無機固定（炭酸化・粗） | 生物起源CO₂の鉱物化。**正味除去はライムLCA次第**（低炭素ライム使用時のみ除去クレジット化可・4-6参照） | 約270 t-CO₂/年 |
...
**前提条件の注記（v2.0試算）**
本仕様書v2.0の発電・熱収支は、主蒸気6 MPa/450℃・ボイラー効率85%・タービン段効率0.78・復水器0.008 MPa（41℃）を想定した設計概念値である。実機の主蒸気条件・タービン効率は施設ごとに異なるため、すべての数値は実装メーカーによる精査前の「参照値」とする。
